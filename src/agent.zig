// flint-agent: guest-side sandbox daemon.
...
// Only one request runs at a time (single-threaded agent).
...
fn handleExec(cmd_str: []const u8, timeout: u32, resp_buf: []u8) []const u8 {
    // Build command for sh -c
    var sh_argv = [_:null]?[*:0]const u8{ "/bin/sh", "-c", null };

    // Copy cmd to null-terminated buffer
    ...
    if (fork_rc == 0) {
        ...
        // Set alarm for timeout (raw syscall — no wrapper in std.os.linux)
        if (timeout > 0) {
    ...
    const stdout_len = readPipe(stdout_fds[0], &exec_stdout);
    _ = linux.close(stdout_fds[0]);

    const stderr_len = readPipe(stderr_fds[0], &exec_stderr);
    _ = linux.close(stderr_fds[0]);
    ...
    // Base64 encode stdout and stderr into static buffers
...
fn readPipe(fd: linux.fd_t, buf: []u8) usize {
    var total: usize = 0;
    while (total < buf.len) {
...
// Static buffers for file I/O — avoids stack overflow
...
fn handleReadFile(path: []const u8, resp_buf: []u8) []const u8 {
    ...
    const file_len = readPipe(fd, &file_buf);
...
fn jsonStr(json: []const u8, key: []const u8) ?[]const u8 {
    ...
    // Search for the key, skipping occurrences inside string values.
    // We only match when the key appears after { or , (top-level).
    const key_pos = blk: {
        var pos: usize = 0;
        while (pos < json.len) {
    ...
    if (json[i] == '"') {
        // String value — skip escaped quotes
        i += 1;
        const start = i;
        while (i < json.len) {
...
pub fn main(_: std.process.Init) !void {
    ...
    const msg = "flint-agent: connecting to host...\n";
    _ = linux.write(1, msg.ptr, msg.len);

    const sock = try connectWithRetry();

    const ready = "flint-agent: connected to host\n";
    _ = linux.write(1, ready.ptr, ready.len);

    // Main command loop — buffers are file-scope statics to avoid stack overflow

    while (true) {
        const payload = recvMsg(sock, &main_msg_buf) catch |err| {
            if (err == error.ConnectionClosed) break;
            // Any other error (e.g. MessageTooLarge) desynchronizes the
            // stream — we can't recover, so disconnect.
            break;
        };
